...
async def join(ctx):
      ...
      await ctx.send('**Succsesfully Joined!!** Now Play Some Music :musical_note:')
      
@bot.command()
async def leave(ctx):
    await ctx.voice_client.disconnect()
    await ctx.send('All Good Things Must Come To An End :wave:')    

@bot.command()
async def play(ctx, *, url):
    await ctx.send(':mag_right: searching for the song........')
...
@bot.command()
async def queue(ctx):
    embedVar = discord.Embed(title="Queue",value=None,color=0x00ff00)
    player = music.get_player(guild_id=ctx.guild.id)
    i=0
